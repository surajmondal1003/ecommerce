...
class PagesController extends Controller
{
    ...
    public function savePage(Request $request){
        ...
        $data['ip_address']=$request->input('inputIP');
        ...
        return response()->json(['message' => 'Succesfully Created Pages','status'=>1]);
	}
    ...
    public function get_pages(Request $request){
        ...
        if(!empty($pages))
        {
            foreach ($pages as $page)
            {
                ...
                $nestedData=array();
                $nestedData[] = $page->page_name;
                $nestedData[] = $page->page_url;
                $nestedData[] = $page->parent_name;
                $nestedData[] = $page->location;
                $nestedData[] = $page->position;
                

                if($page->is_active == 'active')
				$nestedData[] = '<a class="btn btn-sm btn-success" href="javascript:void()" title="Deactivate" onclick="change_page('."'".$page->page_id."'".')"><i class="fa fa-thumbs-up"></i> Active</a>';
                else
				$nestedData[] = '<a class="btn btn-sm btn-danger" href="javascript:void()" title="Activate" onclick="change_page('."'".$page->page_id."'".')"><i class="fa fa-thumbs-down"></i> Inactive</a>';
                ...
                $nestedData[]='<div class="dropdown">
                 ...
                 <div class="dropdown-menu dropdown-menu-right">
                         <a class="dropdown-item" href="admin/update-content/'.$page->page_id.'"><i class="fa fa-eye"></i> Content</a>
                         <a class="dropdown-item" href="admin/update-page-meta/'.$page->page_id.'"><i class="fa fa-eye"></i> Meta</a>
                         <a class="dropdown-item" href="admin/edit-page/'.$page->page_id.'"><i class="fa fa-pencil"></i> Edit</a>
                 </div>
             </div>';
	...
	public function updateContent(Request $request){
        ...
        $id=$request->id;
        $data['page_content']=$request->input('inputContent');
        $data['ip_address']=$request->input('inputIP');
    ...
    private function _validatePage(Request $request){
        ...
        if($request->input('inputUrl') != '')
		{
            $pages=Pages::where('page_url',$request->input('inputUrl'))->first();
            if (count($pages)>0)
            {
                $data['inputerror'][] = 'inputUrl';
