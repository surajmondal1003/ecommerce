...
function displayLatestArrivals(){
    ...
    where pi.is_primary="1" ORDER by p.product_id DESC limit 15'));
    if($result){
        foreach($result as $row){
...
function displaySimilarProducts($caturl){
    ...
    where pi.is_primary="1" AND c.category_url="'.$caturl.'" GROUP by p.product_id DESC limit 10'));
...
function displayMostViewedProducts(){
    
    
    $result = \DB::select(\DB::raw('SELECT pi.image_name,c.category_url,p.product_url,p.product_name,
    p.regular_price,p.discount_price
     FROM product_view_counts as pvc left join
	`products` as p on p.product_id=pvc.product_id
    left join product_images as pi on pi.product_id=p.product_id
    left join productcategory_relations as pc on pc.product_id=p.product_id
    left join categories as c on c.category_id=pc.category_id
    where pi.is_primary="1"  GROUP by p.product_id DESC ORDER BY pvc.count DESC limit 15'));
        ...
        foreach($result as $row){
            ...
            echo '<div class="item">
                    <div class="box hvr-grow-shadow">
                        ...
                        <div class="txt">
                        <a href="'.url($row->category_url.'/'.$row->product_url).'">
                            <h2>'.str_limit($row->product_name,18).'</h2>
                        </a>
                            <div class="ret">
                                <h3 class="lft-price">
                                    <strike>Rs '.number_format($row->regular_price).'</strike>
...
function getpageMetaDetails() {
    $url=ltrim($_SERVER['REQUEST_URI'],'/');
   
    $metaString = "";
     if($url){
             $metaVal =   \DB::select(\DB::raw("select * from page_metas as pm left join pages as p on p.page_id=pm.page_id where p.page_url='".$url."'"));
...
function isPermitted($url,$admin_id){
	
    $result=\DB::select(\DB::raw("select * from admin_permissions where admin_id=".$admin_id." and permission_url='".$url."'"));
	...
	// foreach($result as $row){
    //     $userPermission[]=$row->permission_url;
    // }
